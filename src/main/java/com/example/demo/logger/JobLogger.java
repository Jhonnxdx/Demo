...
import com.example.demo.logger.handler.console.ConsoleLogger;
import com.example.demo.logger.handler.db.DBLogger;
import com.example.demo.logger.handler.file.FileLogger;
...
public class JobLogger implements ILogger{
	...
	public void logMessage(String messageText, Boolean message, Boolean warning, Boolean error)
		...
		int t = 0;
        if (isInfoEnabled()) {
            t = com.example.demo.logger.domain.Level.INFO.getValue();
        }

        if (isErrorEnabled()) {
        	t = com.example.demo.logger.domain.Level.ERROR.getValue();
        }

        if (isWarnEnabled()) {
        	t = com.example.demo.logger.domain.Level.WARN.getValue();
        }
